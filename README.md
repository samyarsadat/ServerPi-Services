...
Many of the services I have set up require certain "secret" variables (passwords, API keys, etc.). I store these secrets in `secrets.env` files located in the root directory of each service (the same directory that contains the `docker-compose.yaml` file). I, of course, have omitted these files from this repository, so you will have to create them yourself before running the desired service. The variables that need to be set in these `secrets.env` files are listed in `secrets.txt` files, located in the root directory of each service ([example](monitoring/secrets.txt)). If there is no `secrets.txt` file in the root directory of a service, then that service has no secret variables.
